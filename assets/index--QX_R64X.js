...
🪙【金運】自分を一番輝かせるためのお洒落、勝負コスメ、大人の香水への投資が最高の金運を呼びます。お財布を開いて新しい自分の魅力を手に入れることで、未来の豊かな財運が力強く動き出します。`}},H=null,U=[],W=1,G=[{id:0,name:`自分自身・本質`,emoji:`☉`},{id:1,name:`私生活・心`,emoji:`☾`},{id:2,name:`知性・会話`,emoji:`☿`},{id:3,name:`恋愛・楽しさ`,emoji:`♀`},{id:4,name:`やる気・行動`,emoji:`♂`},{id:5,name:`拡大・幸運`,emoji:`♃`},{id:6,name:`課題・試練`,emoji:`♄`},{id:7,name:`個性・打破`,emoji:`♅`},{id:8,name:`理想・スピ`,emoji:`♆`},{id:9,name:`底力・リreset`,emoji:`♇`}],K=[{id:0,name:`太陽（舞台）`,emoji:`☉`},{id:2,name:`水星（仕事）`,emoji:`☿`},{id:3,name:`金星（ワクワク）`,emoji:`♀`},{id:4,name:`火星（情熱）`,emoji:`♂`}];async function q(e,t){try{if(H&&typeof H.calculatePosition==`function`){let n=await H.calculatePosition(e,t,0);if(n&&typeof n.longitude==`number`&&!isNaN(n.longitude))return n.longitude}}catch{}let n=(e-2451545)/36525,r=e-2451545,i={0:{L:280.466+36000.77*n,w:282.937,e:.0167},1:{L:218.316+481267.88*n,w:83.353,e:.0549},2:{L:252.251+149472.67*n,w:77.456,e:.2056},3:{L:181.979+58517.815*n,w:131.532,e:.0068},4:{L:355.453+19140.303*n,w:336.041,e:.0934},5:{L:34.404+3034.746*n,w:14.753,e:.0484},6:{L:49.944+1222.114*n,w:92.431,e:.0541},7:{L:313.232+428.482*n,w:170.964,e:.0473},8:{L:304.88+218.459*n,w:44.971,e:.0086},9:{base:240,speed:.004}};if(t===9){let e=(i.base+r*i.speed)%360;return e<0?e+360:e}let a=i[t]||i,o=a.L%360;o<0&&(o+=360);let s=(o-a.w)*Math.PI/180,c=(2*a.e-a.e**3/4)*Math.sin(s)+1.25*a.e**2*Math.sin(2*s),l=o+c*180/Math.PI;if(t===2||t===3||t===4){let e=i.L%360,n=(o-e)*Math.PI/180;l+=(t===2?14:t===3?28:8.5)*Math.sin(n)}return l%=360,l<0?l+360:l}function J(e){let t=e.getUTCFullYear(),n=e.getUTCMonth()+1,r=e.getUTCDate(),i=e.getUTCHours()+e.getUTCMinutes()/60+e.getUTCSeconds()/3600;n<=2&&(--t,n+=12);let a=Math.floor(t/100),o=2-a+Math.floor(a/4);return Math.floor(365.25*(t+4716))+Math.floor(30.6001*(n+1))+r+o-1524.5+i/24}function Y(e,t,n){return n===`上旬`?`1日～10日`:n===`中旬`?`11日～20日`:`21日～${new Date(e,t,0).getDate()}日`}async function X(){let e=document.createElement(`style`);e.textContent=B,document.head.appendChild(e);try{let e=z||o,t=window.location.pathname.endsWith(`/`)?window.location.pathname:window.location.pathname.substring(0,window.location.pathname.lastIndexOf(`/`)+1);H=new e({ephePath:t+`ephe/`,wasmBinaryFile:t+`assets/swisseph-BmP0Bw24.wasm`}),H&&typeof H.init==`function`&&await H.init()}catch{}let t=localStorage.getItem(`astrology_birthdate`)||`1995-10-10T12:00`,n=localStorage.getItem(`astrology_scan_year`)||`2026`,r=document.getElementById(`app`);if(!r)return;r.innerHTML=`
    <!-- 🚗 キャラが爆走するための隠しタグを設置！ -->
    ...
    </div>`;let i=document.getElementById(`calcBtn`);i&&i.addEventListener(`click`,Z),ee()}async function Z(){let e=document.getElementById(`birthdate`).value,t=document.getElementById(`scanyear`).value;if(!e||!t){alert(`入力してね✨`);return}localStorage.setItem(`astrology_birthdate`,e),localStorage.setItem(`astrology_scan_year`,t);let n=document.getElementById(`calcBtn`),r=document.getElementById(`resultSection`),i=document.getElementById(`scanChara`);i&&(i.classList.add(`active`),setTimeout(()=>{i.classList.remove(`active`)},1500)),n.disabled=!0,r.style.display=`none`,U=[];let a=G,o=J(new Date(e)),s={};for(let e=0;e<a.length;e++){let t=a[e],n=await q(o,t.id);s[String(t.id)]=typeof n==`number`&&!isNaN(n)?n:0}let c=parseInt(t,10),l=c%4==0&&c%100!=0||c%400==0?366:365,u=new Date(c,0,1,12,0,0),d=J(u),f={0:5,2:2.5,3:3,4:4},p=[0,2,3,4],m=[{angle:0,name:`合 (0°)`,emoji:`☌`,type:`good`},{angle:120,name:`トライン`,emoji:`△`,type:`good`},{angle:90,name:`スクエア`,emoji:`□`,type:`bad`},{angle:180,name:`オポ`,emoji:`☍`,type:`bad`}],h=[],g={};for(let e=1;e<=12;e++)[`上旬`,`中旬`,`下旬`].forEach(t=>{let n=`${e}_${t}`;h.push(n),g[n]={monthNum:e,periodName:t,rangeStr:Y(c,e,t),allAspects:[]}});for(let e=0;e<l;e++){e%15==0&&await new Promise(e=>setTimeout(e,1));let t=d+e,n=new Date(u.getTime()+e*24*60*60*1e3),r=n.getMonth()+1,i=n.getDate(),o=`下旬`;i<=10?o=`上旬`:i<=20&&(o=`中旬`);let c=g[`${r}_${o}`];if(c)for(let e=0;e<p.length;e++){let n=p[e],r=K.find(e=>e.id===n),o=await q(t,n),l=f[n]||3;for(let e=0;e<a.length;e++){let t=a[e],u=s[String(t.id)],d=Math.abs(o-u);d>180&&(d=360-d);for(let e=0;e<m.length;e++){let a=m[e],o=Math.abs(d-a.angle);if(o<=l){let e=V[`${n}_${t.id}_${a.angle}`];e&&c.allAspects.push({transitPlanet:r,natalPlanet:t,aspect:a,score:o,dayNumber:i,title:e.title,desc:e.desc})}}}}}for(let e of h){let t=g[e],n=t.monthNum,r=t.periodName===`上旬`?1:t.periodName===`中旬`?11:21,i=t.periodName===`上旬`?10:t.periodName===`中旬`?20:new Date(c,n,0).getDate();if(t.allAspects.length===0)for(let e=r;e<=i;e++){let r=J(new Date(c,n-1,e,12,0,0));for(let n of p){let i=K.find(e=>e.id===n),o=await q(r,n);for(let r of a){let a=s[String(r.id)],c=Math.abs(o-a);c>180&&(c=360-c);for(let a of m){let o=Math.abs(c-a.angle);if(o<=8){let s=V[`${n}_${r.id}_${a.angle}`];s&&t.allAspects.push({transitPlanet:i,natalPlanet:r,aspect:a,score:o,dayNumber:e,title:s.title,desc:s.desc})}}}}}let o={};t.allAspects.forEach(e=>{if(!e||!e.transitPlanet||!e.natalPlanet||!e.aspect)return;let t=`${e.transitPlanet.id}_${e.natalPlanet.id}_${e.aspect.angle}_${e.dayNumber}`;(!o[t]||e.score<o[t].score)&&(o[t]=e)}),t.allAspects=Object.values(o),t.allAspects.length>0?(t.allAspects.sort((e,t)=>e.score-t.score),t.allAspects.slice(0,2).forEach(e=>{U.push({transitPlanet:e.transitPlanet,natalPlanet:e.natalPlanet,aspect:e.aspect,title:e.title||``,desc:e.desc||``,monthNum:t.monthNum,periodName:t.periodName,rangeStr:t.rangeStr,peakDay:e.dayNumber,isFlat:!1})})):U.push({transitPlanet:{id:99,name:`星の余白`,emoji:`🌙`},natalPlanet:{id:99,name:`あなたの心`,emoji:`✨`},aspect:{angle:99,name:`フラット`,emoji:`🍃`,type:`flat`},title:`星たちの穏やかな余白期間。特に何もない日はフラットにいこう`,desc:`☕ 今の空はおだやかで、あなたの星を強く揺さぶるようなピリピリした角度はありません。こういう時期は、焦って大きな行動を起こさなくても大丈夫。いつもより少し長めにお風呂に入ったり、お気に入りの温かいハーブティーを飲んで、自分の心と体をただ優しくフラットに整えてあげるのが一番の開運アクションです。`,monthNum:t.monthNum,periodName:t.periodName,rangeStr:t.rangeStr,peakDay:r,isFlat:!0})}U.sort((e,t)=>e.monthNum===t.monthNum?e.peakDay-t.peakDay:e.monthNum-t.monthNum),U.forEach(e=>{e.dateRangeLabel=`${e.monthNum}月${e.periodName}`,e.peakDayLabel=e.isFlat?`${e.monthNum}月${e.periodName}全体`:`${e.monthNum}月${e.peakDay}日頃`}),W=1,Q(),r.style.display=`block`,n.disabled=!1,$(U)}function Q(){let e=document.getElementById(`monthTabs`);if(!e)return;let t=``;for(let e=1;e<=12;e++)t+=`<button class="month-tab-btn ${e===W?`active`:``}" data-month="${e}">${e}月</button>`;e.innerHTML=t,e.querySelectorAll(`.month-tab-btn`).forEach(t=>{t.addEventListener(`click`,t=>{e.querySelectorAll(`.month-tab-btn`).forEach(e=>e.classList.remove(`active`)),t.target.classList.add(`active`),W=parseInt(t.target.getAttribute(`data-month`),10),$(U);let n=document.getElementById(`filterTabs`);n&&(n.querySelectorAll(`.tab-btn`).forEach(e=>e.classList.remove(`active`)),n.querySelector(`[data-type="all"]`).classList.add(`active`))})})}function $(e){let t=document.getElementById(`scheduleList`);if(!t)return;let n=e.filter(e=>e&&e.monthNum===W);if(n.length===0){t.innerHTML=`<div class="no-data">この月のデータは見つかりませんでした。</div>`;return}let r=``,i=null;n.forEach(e=>{if(!e||!e.aspect||!e.transitPlanet||!e.natalPlanet)return;let t=e.aspect.type===`good`,a=e.aspect.type===`flat`,o=(e.desc||``).replace(/\n/g,`<br>`),s=e.monthNum;if(s!==i){i=s;let e=n.map(e=>({transitPlanetId:e.transitPlanet.id,title:e.title,rawDate:new Date(2026,s-1,1)})),t=``;typeof getMonthlyThemeTitle==`function`&&(t=getMonthlyThemeTitle(e,s-1));let a=t&&t.trim()!==``&&!t.includes(`余白月`)&&!t.includes(`undefined`);r+=`
        <div class="monthly-header-divider">
          <h2>🌙 ${s}月の星</h2>
          ${a?`<p class="monthly-sub">${t}</p>`:``}
        </div>`}let c=`#ff69b4`,l=`💖 好調期`;a?(c=`#a7bfae`,l=`🍃 フラット期`):t||(c=`#ffd700`,l=`⚠️ 注意期`),r+=`
